...
class BaseLoader:
    ...
    def __init__(self, filename: str, dtypes_dict: dict = None):
        ...
        self.df_indices['short pass'] = self.df[(self.df['play_type'] == 'pass') & (self.df['air_yards'] <= 40)].index
        self.df_indices['long pass'] = self.df[(self.df['play_type'] == 'pass') & (self.df['air_yards'] > 40)].index
        self.df_indices['incomplete_pass'] = self.df[self.df['play_type'] == 'incomplete_pass'].index
    ...
    @abc.abstractmethod
    def get_probability(self, down: int, to_gl: int, position: int, play: PlayType) -> List[PlayOutcome]:
...
class DownOnlyLoader(BaseLoader):
    ...
    def get_probability(self, down: int, to_go: int, play: PlayType, position: int = None) -> List[PlayOutcome]:
        ...
        elif play is PlayType.DEEPPASS:
            data = self.df[(self.df['play_type'] == 'pass') &  (self.df['air_yards'] > 40)]
        elif play is PlayType.SHORTPASS:
            data = self.df[(self.df['play_type'] == 'pass') & (self.df['air_yards'] <= 40)]
        else:
        ...
        def determine_outcome(row: pd.Series):
            ...
            elif row['incomplete_pass']:
                outcome = OutcomeType.INCOMPLETE
            else:
...
class FieldPositionLoader(BaseLoader):
    ...
    def get_probability(self, down: int, to_go: int, position: int,
        ...
        if play is PlayType.QB_SNEAK:
            ...
            data = self.df[(self.df['play_type'] == 'pass') & (self.df['air_yards'] <= 40)]
        elif play is PlayType.FIELD_GOAL:
...
if __name__ == '__main__':
    ...
    print('Prob')
    outcomes = fol.get_probability(2, 8, 80, PlayType.SHORTPASS)
    for o in outcomes:
        print(o)
    print('\n\n')
